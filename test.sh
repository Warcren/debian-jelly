...
run_nala_installPackages() {
	...
	# Add the non-free repository to the sources.list file
	echo "deb http://deb.debian.org/debian buster main non-free" | sudo tee -a /etc/apt/sources.list
...
echo "hardwareAcceleration.enableVAAPI = true" | sudo tee -a /etc/jellyfin/encoding.xml

# Define the device path for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiDevicePath = \"/dev/dri/renderD128\"" | sudo tee -a /etc/jellyfin/encoding.xml

# Define the driver type for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiDriverTypeOverride = \"i965\"" | sudo tee -a /etc/jellyfin/encoding.xml

# Drop advanced subtitles for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiDropAdvancedSubtitlesOverride = true" | sudo tee -a /etc/jellyfin/encoding.xml

# Define the output format for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiHwaccelOutputFormatOverride = \"vaapi_vld\"" | sudo tee -a /etc/jellyfin/encoding.xml

# Allow hwaccel transcoding for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiAllowHwaccelTranscodingOverride = true" | sudo tee -a /etc/jellyfin/encoding.xml

# Allow hwaccel decoder for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiAllowHwaccelDecoderOverride = true" | sudo tee -a /etc/jellyfin/encoding.xml

# Allow hwaccel encoder for VAAPI (requires Intel GPU)
echo "hardwareAcceleration.vaapiAllowHwaccelEncoderOverride = true" | sudo tee -a /etc/jellyfin/encoding.xml


#Create Jellyfin files
sudo touch /var/lib/jellyfin
sudo touch /etc/jellyfin
sudo touch /usr/share/jellyfin
sudo touch /var/log/jellyfin
...
ExecStart=ExecStart=/bin/bash -c 'export HOME=$(getent passwd $USER | cut -d: -f6); exec $HOME/.nix-profile/bin/jellyfin'
Restart=on-failure
RestartSec=5
